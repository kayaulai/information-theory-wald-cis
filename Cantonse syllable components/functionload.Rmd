...
aps.wf_counts = aps.wf %>% group_by(o,n,c,t,file) %>% summarise(count = n())
```
...
```{r}
getFLDiffsFromFakeCorpus(fakeCorpora_w[[1]] , cmatrix)
...
shapiro.test(simFLDiffMatrix_w[,1])
shapiro.test(simFLDiffMatrix_w[,2])
shapiro.test(simFLDiffMatrix_w[,3])
shapiro.test(simFLDiffMatrix_w[,4])
shapiro.test(simFLDiffMatrix_w[,5])
shapiro.test(simFLDiffMatrix_w[,6])
